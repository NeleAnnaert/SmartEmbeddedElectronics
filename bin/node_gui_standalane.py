...
def button_1(*args, **kwargs):
    rospy.wait_for_service('color_calibration')  # The service name
    try:
        service = rospy.ServiceProxy('color_calibration', Trigger)  # Create service function
...
def button_2(*args, **kwargs):
    rospy.wait_for_service('perspective_calibration')  # The service name
    try:
        service = rospy.ServiceProxy('perspective_calibration', Trigger)  # Create service function
        response = service()  # Run service function
        return response  # Return the response
    except rospy.ServiceException:
        pass
    ...
    while True:
        if not os.path.isfile(mapper_lock_file):
            if os.path.isfile(mapper_file):
                ...
                parking_loc = Location(data.parking_loc_x, data.parking_loc_y, data.parking_loc_angle)
                previous_park = Location(data.previous_park_x, data.previous_park_y, data.previous_park_angle)
                weight = data.weight
                send_command = data.send_command
                app.update_mapper(map_matrix, current_loc, previous_loc, parking_loc, previous_park, weight,
                                  send_command)
